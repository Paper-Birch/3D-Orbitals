...
def orbital_probability(n, l, m, r_array, phi_array, theta_array):
    ...
    wave_func = np.sqrt(((2 / (n * a_0))**3) * (np.math.factorial(n - l - 1) / np.math.factorial(2 * n * (n + l)))) * radial_comp * special.sph_harm(m, l, phi_array, theta_array)
    ...
    # Return the full wavefunction probability distribution as well as its radial component and phase
    return waveprob, radprob, np.angle(wave_func)
...
if 0 < n <= 4 and 0 <= l < n and -l <= m <= l:
    ...
    # Get the probability arrays and phase
    waveprob, radprob, phase = orbital_probability(n, l, m, r_array, phi_array, theta_array)
    ...
    prob_list_normalized[prob_list_normalized<0.1]=0
    ...
    prob_plot = ax1.scatter(x_coords, y_coords, z_coords, c=phase_list, cmap='viridis', alpha=opacity_list, s=(prob_list_normalized * 100))
    ...
    ax1.set_title(f'Probability Distribution and Phase\nfor an Electron in the n={n} l={l} m\u2097={m} State')
    ...
    # Add a colorbar
    fig.colorbar(prob_plot, orientation="horizontal", ax=ax1, label='Phase (radians)')
